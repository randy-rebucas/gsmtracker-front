...
if ($.navAsAjax) {
    ...
    $(document).on('click', '.sidebar-menu li a[target="_top"]', function(e) {
        e.preventDefault();
        var $this = $(e.currentTarget);
    ...
    $(document).on('click', '.sidebar-menu li a[href!="#"], .ajaxify', function(e) {

        history.pushState(null, null, this.href);
        checkURL();

        // $('nav li:has(a[href="' + this.href + '"])').removeClass("open").addClass("active");
        var title = ($('.sidebar-menu li a[href="' + this.href + '"]').attr('title'));
...
function replacePage(url, container) {
    ...
    $.ajax({
        ...
        beforeSend: function() {
            // cog placed
            container.html('<div data-role="activity" data-type="cycle" data-style="color"></div>');
