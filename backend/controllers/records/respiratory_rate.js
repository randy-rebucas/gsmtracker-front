...
exports.getCurrent = async(req, res, next) => {
    try {
        ...
        let respiratoryrate = await RespiratoryRate.findOne({
            ...
            .select('respiratoryrate created')
            .exec();
...
exports.getLast = async(req, res, next) => {
    try {
        ...
        let respiratoryrate = await RespiratoryRate.findOne({ 'patientId': req.params.patientId }, projection)
            .select('respiratoryrate created')
            .sort({ created: -1 })
            .exec();
