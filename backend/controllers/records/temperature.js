...
exports.getCurrent = async(req, res, next) => {
    try {
        ...
        let temperature = await Temperature.findOne({
...
exports.getLast = async(req, res, next) => {
    try {
        ...
        let temperature = await Temperature.findOne({ 'patientId': req.params.patientId }, projection)
            .select('temperature created')
            .sort({ created: -1 })
            .exec();
