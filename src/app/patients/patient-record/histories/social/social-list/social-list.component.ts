...
implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.recordsSub = this.socialService
      .getUpdateListener()
      .subscribe((socialData: {socials: SocialData[], count: number}) => {
        this.isLoading = false;
        this.total = socialData.count;
        this.dataSource = new MatTableDataSource(socialData.socials);
  ...
  onDelete(progressNoteId) {
    ...
    .afterClosed().subscribe(res => {
      if (res) {
        this.socialService.delete(progressNoteId).subscribe(() => {
