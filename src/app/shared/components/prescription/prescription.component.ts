...
export class PrescriptionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.patientService.getByKey(this.block.transactions.toAddress)
    .pipe(
      map(userData => {
        const userId = {
          id: userData._id,
  ...
  onPrint() {
    ...
    .subscribe((res) => {
      this.translate.get([
        ...
        // header
        if (setting.rxHeaderOption) {
          // title
          pdfDoc.setFontSize(16);
          pdfDoc.setFont('normal');
          pdfDoc.setFontType('italic');
          pdfDoc.text(setting.prescription.rxTitle, pageWidth / 2, 20, null, null, 'center');
          // subtitle
          pdfDoc.setFontSize(14);
          pdfDoc.setFont('courier');
          pdfDoc.setFontType('regular');
          pdfDoc.text(setting.prescription.rxSubTitle, pageWidth / 2, 26, null, null, 'center');
          // clinic address
          pdfDoc.setFontSize(10);
          pdfDoc.setFont('courier');
          pdfDoc.setFontType('regular');
          setting.prescription.rxAddresses.forEach(element => {
          ...
          pdfDoc.setFontType('regular');
          const contacts = [];
          setting.prescription.rxPhones.forEach(element => {
            contacts.push(element.contact);
          });
          pdfDoc.text(translation['basic.contact'] + ' ' + contacts.join('/'), 95, 47, null, null, 'left');
          // clinic hour text
          pdfDoc.setFontSize(10);
          pdfDoc.setFont('courier');
          pdfDoc.setFontType('regular');
          pdfDoc.text(translation['settings.clinic-hours'], 95, 35, null, null, 'left');
          // clinic hour val
          pdfDoc.setFontSize(10);
          pdfDoc.setFont('courier');
          pdfDoc.setFontType('regular');
          const hours = [];
          setting.prescription.rxHours.forEach(element => {
          ...
          if (setting.prescription.rxNoNoonBreak) {
            pdfDoc.setFontSize(10);
            pdfDoc.setFont('courier');
            pdfDoc.setFontType('regular');
            pdfDoc.text(translation['settings.no-noon-break'], 95, 43, null, null, 'left');
          }
        }
        // line
        ...
        pdfDoc.setFont('courier');
        pdfDoc.setFontType('regular');
        pdfDoc.text(translation['basic.name'], 10, 54, null, null, 'left');
        // patient fullname
        pdfDoc.setFontSize(10);
        pdfDoc.setFont('courier');
        pdfDoc.setFontType('regular');
        pdfDoc.text(this.user.name.firstname + ' ' + this.user.name.midlename + ' ' + this.user.name.lastname, 35, 54, null, null, 'left');
        // birthdate text
        pdfDoc.setFontSize(10);
        pdfDoc.setFont('courier');
        pdfDoc.setFontType('regular');
        pdfDoc.text(translation['basic.birthdate'], 95, 54, null, null, 'left');
        // birthdate
        pdfDoc.setFontSize(10);
        pdfDoc.setFont('courier');
        pdfDoc.setFontType('regular');
        const dob = datePipe.transform(this.user.birthdate, 'MMM dd, yyyy');
        ...
        pdfDoc.setFont('courier');
        pdfDoc.setFontType('regular');
        pdfDoc.text(translation['basic.age'], 10, 58, null, null, 'left');
        // patient age val
        pdfDoc.setFontSize(10);
        pdfDoc.setFont('courier');
        pdfDoc.setFontType('regular');
        pdfDoc.text(' ' + this.getAge(new Date(this.user.birthdate)) + '', 35, 58, null, null, 'left');
        // gender text
        pdfDoc.setFontSize(10);
        pdfDoc.setFont('courier');
        pdfDoc.setFontType('regular');
        pdfDoc.text(translation['basic.gender'], 95, 58, null, null, 'left');
        // gender val
        pdfDoc.setFontSize(10);
        pdfDoc.setFont('courier');
        pdfDoc.setFontType('regular');
        pdfDoc.text(this.user.gender, 115, 58, null, null, 'left');
        // patient address text
        pdfDoc.setFontSize(10);
        pdfDoc.setFont('courier');
        pdfDoc.setFontType('regular');
        pdfDoc.text(translation['basic.address'], 10, 62, null, null, 'left');
        // patient address
        pdfDoc.setFontSize(10);
        pdfDoc.setFont('courier');
        pdfDoc.setFontType('regular');
        this.user.addresses.forEach(element => {
          if (element.current) {
            pdfDoc.text(element.address1, 35, 62, null, null, 'left');
            let gap = 0;
            if (element.address2) {
              gap = 4;
              pdfDoc.text(element.address2, 35, 66, null, null, 'left');
            }
            pdfDoc.text('' + element.postalCode + '', 35, 66 + gap, null, null, 'left');
            pdfDoc.text(element.province, 45, 66 + gap, null, null, 'left');
            pdfDoc.text(element.city, 58, 66 + gap, null, null, 'left');
            pdfDoc.text(element.country, 35, 70 + gap, null, null, 'left');
          }
        });
        ...
        pdfDoc.setFont('courier');
        pdfDoc.setFontType('regular');
        pdfDoc.text(translation['basic.date'], 95, 62, null, null, 'left');
        // date val
        pdfDoc.setFontSize(10);
        pdfDoc.setFont('courier');
        pdfDoc.setFontType('regular');
        pdfDoc.text(datePipe.transform(new Date(), 'MMM dd, yyyy'), 115, 62, null, null, 'left');
        // rx
        ...
        for (let index = 0; index < this.prescriptions.length; index++) {
          ...
          pdfDoc.text('' + num + '', 10, 94 + ( index * 10 ), null, null, 'left');
          pdfDoc.setFontType('regular');
          pdfDoc.setTextColor(0, 0, 0);
          pdfDoc.text( element.medicine + ' ' + element.preparation, 15, 94 + ( index * 10 ), null, null, 'left');
          pdfDoc.setFontType('regular');
          pdfDoc.setTextColor(0, 0, 0);
          pdfDoc.text(' # ' + element.quantity, 130, 94 + ( index * 10 ), null, null, 'left');
          pdfDoc.setFontType('italic');
          pdfDoc.setTextColor(51, 122, 183);
          pdfDoc.text(element.sig, 20, 94 + ( index * 10 ) + 5, null, null, 'left');
        }
        // doctor
        pdfDoc.setFontSize(10);
        pdfDoc.setFont('courier');
        ...
        pdfDoc.text(
          ...
          this.user.name.midlename + '.' +
          this.user.name.lastname + ' M.D.', 135, 180, null, null, 'right');
        // prc label
        pdfDoc.setFontSize(10);
        pdfDoc.setFont('courier');
        pdfDoc.setFontType('regular');
        pdfDoc.setTextColor(0, 0, 0);
        pdfDoc.text(translation['physicians.prc'], 120, 185, null, null, 'right');
        // prc number
        pdfDoc.setFontSize(10);
        pdfDoc.setFont('courier');
        pdfDoc.setFontType('regular');
        ...
        pdfDoc.setFontType('regular');
        pdfDoc.setTextColor(0, 0, 0);
        pdfDoc.text(translation['physicians.ptr'], 120, 190, null, null, 'right');
        // ptr number
        pdfDoc.setFontSize(10);
        pdfDoc.setFont('courier');
        pdfDoc.setFontType('regular');
        ...
        pdfDoc.setFontType('regular');
        pdfDoc.setTextColor(0, 0, 0);
        pdfDoc.text(translation['physicians.s2'], 120, 195, null, null, 'right');
        // s2 number
        pdfDoc.setFontSize(10);
        pdfDoc.setFont('courier');
        pdfDoc.setFontType('regular');
